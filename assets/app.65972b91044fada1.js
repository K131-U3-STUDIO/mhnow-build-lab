...
function renderSkillCatalog(){
 ...
 $('skillCheckList').innerHTML=names.slice(0,160).map(k=>{const d=DB.skills[k]||{},max=d.max||5,mode=skillConditionMode(k),active=!!mode,sel=mode==='required'?requirements[k]:mode==='max'?skillLimits[k]:1,av=skillAvailability(k);let opts='';for(let n=1;n<=max;n++)opts+=`<option value="${n}" ${sel===n?'selected':''}>Lv${n}</option>`;return `<div class="skill-row ${active?'selected':''} ${mode==='exclude'?'excluded':''} ${mode==='max'?'maxed':''}" data-skill="${esc(k)}"><input class="skill-check" type="checkbox" aria-label="${esc(k)}を条件に追加" ${active?'checked':''}><div class="skill-info"><div class="sname">${esc(k)}</div><div class="smeta">防具 ${av.a} / 武器 ${av.w}${d.drift?' / 漂移可':''}</div></div><div class="skill-controls"><label class="skill-control"><span class="skill-control-label">条件</span><select class="skill-mode" aria-label="${esc(k)}の条件"><option value="required" ${mode==='required'||!mode?'selected':''}>必須</option><option value="exclude" ${mode==='exclude'?'selected':''}>除外</option><option value="max" ${mode==='max'?'selected':''}>上限</option></select></label><label class="skill-control"><span class="skill-control-label">レベル</span><select class="skill-lv" aria-label="${esc(k)}のレベル" ${mode==='exclude'?'disabled':''}>${opts}</select></label></div></div>`}).join('');
...
function enumerate(req={},progressCb=null,limits=skillLimits){
 ...
 return new Promise((resolve,reject)=>{
  const worker=new Worker("./assets/search-worker.d6296309166c54fd.js");let timer,settled=false;
